...
add column if not exists source_invoice_transaction_id uuid
references public.transactions(id) on delete set null;

create index if not exists credit_card_purchases_source_invoice_transaction_idx
on public.credit_card_purchases(user_id, source_invoice_transaction_id)
where source_invoice_transaction_id is not null;
